...
(use-package emacs
  :ensure nil
  :preface
  (defun +ispell-load-configuration ()
    (require 'ispell)
    ...
    (defvar +ispell-backend nil)

    (defun +ispell-load-hunspell-configs ()
      ...
      (setq +ispell-backend "hunspell"))

    (defun +ispell-load-aspell-configs ()
      "Load aspell spelling backend."
      (setq ispell-program-name "/usr/bin/aspell")
      ;; Please note ispell-extra-args contains ACTUAL parameters passed to aspell
      (setq ispell-extra-args '("-a" "soundslike" "--sug-mode=ultra" "--lang=en_US"))
      (setq +ispell-backend "aspell"))

    (cond ((executable-find "hunspell") (+ispell-load-hunspell-configs))
          ((executable-find "aspell") (+ispell-load-aspell-configs)))
...
Also position fit window to BUFFER and select it."
  ...
  :hook
  (elpaca-after-init . +ispell-load-configuration))
